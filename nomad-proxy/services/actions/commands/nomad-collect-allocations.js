...
const handler = async function (ctx) {
  try {
    ...
    // get all allocations
    const { data } = await axios.get(`${nomad.addr}/v1/allocations?namespace=*`, options)
    ...
    // write data
    await fse.writeJSON(`${cache.directory}/allocations.json`, result)
    return { success: true }
  } catch (e) {
...
module.exports = {
  rest: 'POST /commands/collect-allocations',
  handler
}
